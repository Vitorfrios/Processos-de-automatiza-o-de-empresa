...
class UniversalDeleteModal {
 ...
 `;
  ...
  setupEventListeners() {
    ...
    document
      .getElementById("universal-delete-cancel")
      .addEventListener("click", () => this.hide());
    document
      .getElementById("universal-delete-confirm")
      .addEventListener("click", async () => {
        if (this.currentDeletion?.callback)
 ...
 `;
    ...
    // Configurar ESC
    this.escHandler = (e) =>
      e.key === "Escape" && this.isShowing && this.hide();
    document.addEventListener("keydown", this.escHandler);

    // Focar no botão de cancelar
    setTimeout(
      () => document.getElementById("universal-delete-cancel").focus(),
      12,
    );
