...
const FilterSystem = (function() {
    ...
    function updateSwitchUI(isActive) {
        ...
        if (label) {
            label.textContent = isActive 
                ...
                : 'Filtro de Obras';
            label.style.color = isActive ? '#4CAF50' : '';
            label.style.fontWeight = isActive ? 'bold' : '';
        }
    ...
    async function loadObraIntoDOM(obraData) {
        try {
            ...
            // Tentar usar sistema existente de várias formas
            
            // Opção 1: Usar createEmptyObra + populateObraData
            if (typeof window.createEmptyObra === 'function' && typeof window.populateObraData === 'function') {
                console.log(`🔨 [FILTER-SYSTEM] Criando obra via createEmptyObra`);
            ...
            // Opção 2: Usar função loadSingleObra se existir
            else if (typeof window.loadSingleObra === 'function') {
                console.log(`🔨 [FILTER-SYSTEM] Carregando obra via loadSingleObra`);
                await window.loadSingleObra(obraData);
            }
            // Opção 3: Criar manualmente (fallback extremo)
            else {
                console.error(`⚠️ [FILTER-SYSTEM] Nenhuma função de carregamento encontrada para obra ${obraData.id}`);
    ...
    async function loadSessionObras() {
        ...
        try {
            // Limpar cache quando voltar para modo sessão
            state.currentObras = [];
            
            // Usar função existente se disponível
            if (typeof window.loadObrasFromServer === 'function') {
                await window.loadObrasFromServer();
                console.log('✅ [FILTER-SYSTEM] Obras da sessão carregadas via loadObrasFromServer');
            } else if (typeof loadObrasFromServer === 'function') {
                await loadObrasFromServer();
                console.log('✅ [FILTER-SYSTEM] Obras da sessão carregadas');
            } else {
                console.warn('⚠️ [FILTER-SYSTEM] loadObrasFromServer não encontrada, usando fallback');
                
                // Fallback: carregar diretamente
                const response = await fetch('/api/session-obras');
                if (response.ok) {
                    const sessionData = await response.json();
                    console.log(`📊 [FILTER-SYSTEM] Sessão com ${sessionData.obras?.length || 0} obras`);
                }
            }
        } catch (error) {
