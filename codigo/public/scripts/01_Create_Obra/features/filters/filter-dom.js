...
const FilterDOM = (function () {
  ...
  function validateElements() {
    ...
    if (!allExist) {
      console.warn(" [FILTER-DOM] Alguns elementos não encontrados:", {
  ...
  function setFiltersEnabled(enabled) {
    console.log(
      ` [FILTER-DOM] ${enabled ? "Habilitando" : "Desabilitando"} inputs`,
    );

    // Controlar VISIBILIDADE além de habilitação
    [empresaInput, numeroClienteInput, nomeObraInput].forEach((input) => {
      if (input) {
        // Controlar display/visibility
        if (enabled) {
  ...
  function setupInputListeners() {
    ...
    // Usar 'input' em vez de 'change' para empresa
    // Para capturar seleção do autocomplete e digitação manual
    empresaInput.addEventListener(
      "input",
      debounce(function (e) {
    ...
    numeroClienteInput.addEventListener(
      "input",
      debounce(function (e) {
        const value = e.target.value.trim();
    ...
    nomeObraInput.addEventListener(
      "input",
      debounce(function (e) {
        const value = e.target.value.trim();

        console.log(` [FILTER-DOM] Nome obra alterado: "${value}"`);
