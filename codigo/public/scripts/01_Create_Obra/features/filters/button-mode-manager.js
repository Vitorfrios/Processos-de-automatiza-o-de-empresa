...
class ButtonModeManager {
  ...
  enableFilterMode() {
    ...
    console.log(" [BUTTON-MANAGER] Ativando modo filtro...");
  ...
  disableFilterMode() {
    ...
    console.log(" [BUTTON-MANAGER] Desativando modo filtro...");
  ...
  disableUniversalDeleteButtons() {
    if (
      window.ButtonDeleteUniversal &&
      window.ButtonDeleteUniversal.restoreOriginalButtons
    ) {
      const restoredCount =
        window.ButtonDeleteUniversal.restoreOriginalButtons();
      console.log(
        ` [BUTTON-MANAGER] ${restoredCount} botões de obra universais desativados`,
      );
    } else {
      // Fallback manual
      ...
      console.log(
        ` [BUTTON-MANAGER] ${deleteButtons.length} botões de obra restaurados manualmente`,
      );
    }
  }
  ...
  changeButtonTexts(newText) {
    ...
    allButtons.forEach((button) => {
      ...
      if (text && text.includes("Obra") && onclick.includes("deleteObra")) {
        ...
        console.log(
          ` Texto alterado: "${this.state.originalTexts.get(button)}" → "${button.textContent}"`,
        );
      }
    });
   ...
   */
  applyMode() {
    console.log(" [BUTTON-MANAGER] Aplicando modo atual...");
