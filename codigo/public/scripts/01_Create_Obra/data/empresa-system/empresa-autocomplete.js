...
async function inicializarInputEmpresaHibrido(obraId) {
  ...
  input.addEventListener("focus", function () {
    ...
    if (valorAtual.length === 0) {
      ...
      empresaJaSelecionada &&
      valorAtual ===
        `${this.dataset.siglaSelecionada} - ${this.dataset.nomeSelecionado}`
    ) {
 ...
 */
function processarInputEmpresa(
  termo,
  input,
  dropdown,
  optionsContainer,
  ...
  console.log(
    ` [INPUT] Processando: "${termo}" | Apagando: ${window.usuarioEstaApagando}`,
  );
  ...
  if (sugestoes.length === 1 && termo.length >= 3) {
    // Mínimo 3 caracteres
...
function exibirSugestoes(sugestoes, container, input, dropdown, obraId) {
  ...
  if (
    empresaJaSelecionada &&
    valorAtual ===
      `${input.dataset.siglaSelecionada} - ${input.dataset.nomeSelecionado}`
  ) {
    container.innerHTML = "";
    dropdown.style.display = "none";
    return;
  ...
  if (
    sugestoesLimitadas.length === 1 &&
    valorAtual.length > 0 &&
    !window.usuarioEstaApagando
  ) {
  ...
  container.innerHTML = sugestoesLimitadas
    .map((empresaObj) => {
      const empresa = normalizeEmpresa(empresaObj);
 ...
 `;
  ...
  // APLICAR EVENT LISTENERS DIRETOS PARA SUPORTE TOUCHPAD
...
function exibirTodasEmpresas(empresas, container, input, dropdown, obraId) {
 ...
 `;
  ...
  console.log(
    ` [EMPRESA] Exibindo ${empresasLimitadas.length} de ${empresas.length} empresas`,
  );
}
...
function navegarDropdown(direcao, container, input, dropdown, obraId) {
  ...
  if (activeOption) {
    ...
    console.log(
      ` Navegação: ${currentIndex} → ${nextIndex} (total: ${options.length})`,
    );
  } else {
  ...
  console.log(
    ` Navegando para: ${sigla} - ${nome} (${nextIndex + 1}/${options.length})`,
  );
}
...
function selecionarEmpresa(
  ...
  setTimeout(() => {
    if (
      ...
      typeof window.empresaCadastro.calcularNumeroClienteFinal === "function"
    ) {
  ...
  setTimeout(() => {
    if (obraElement) {
      const formEmpresa = obraElement.querySelector(
...
function selecionarOpcaoAtiva(container, input, dropdown, obraId) {
  ...
  if (activeOption) {
    ...
    console.log(" Seleção por teclado");
...
function aplicarEventListenersDiretos(container, input, dropdown, obraId) {
  ...
  options.forEach((option) => {
    ...
    option._pointerHandler &&
      option.removeEventListener("pointerdown", option._pointerHandler);
    option._touchHandler &&
      option.removeEventListener("touchend", option._touchHandler);
    ...
    option._clickHandler = function (e) {
      ...
      console.log(" Seleção por CLICK direto");
      selecionarEmpresa(sigla, nome, input, dropdown, obraId, "manual");
    };

    // Handler para pointer (touchpad)
    option._pointerHandler = function (e) {
      e.preventDefault();
      e.stopPropagation();
      const sigla = this.dataset.sigla;
      const nome = this.dataset.nome;
      console.log(" Seleção por POINTER direto");
      selecionarEmpresa(sigla, nome, input, dropdown, obraId, "manual");
    };
    ...
    option._touchHandler = function (e) {
      ...
      const sigla = this.dataset.sigla;
      const nome = this.dataset.nome;
      console.log(" Seleção por TOUCH direto");
      selecionarEmpresa(sigla, nome, input, dropdown, obraId, "manual");
    };
 ...
 * Suporte para touchpad, mouse e touch
 */
    ...
    document.addEventListener(eventType, function (e) {
      ...
      if (option) {
        ...
        if (input && dropdown) {
          ...
          console.log(` Seleção por ${eventType}`);
