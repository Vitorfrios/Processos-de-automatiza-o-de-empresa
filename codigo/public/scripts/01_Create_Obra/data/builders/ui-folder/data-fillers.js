...
function fillClimatizationInputs(roomElement, inputsData) {
  ...
  const ambienteInput = roomElement.querySelector(
    `input[data-field="ambiente"]`,
  );
  if (
    ambienteInput &&
    (!inputsData.ambiente || inputsData.ambiente === "") &&
    roomName
  ) {
    inputsData.ambiente = roomName;
  }

  // Encontrar a SEÇÃO DA TABELA, não a seção de climatização
  const tableSection = roomElement.querySelector(
    `#section-content-${roomId}-input-table`,
  );
  ...
  if (inputsData.pressurizacao !== undefined) {
    const isPressurizacaoAtiva =
      typeof inputsData.pressurizacao === "boolean"
        ? inputsData.pressurizacao
        : inputsData.pressurizacao === "true" ||
          inputsData.pressurizacao === true ||
          inputsData.pressurizacao === 1;

    const pressurizacaoValue = isPressurizacaoAtiva ? "sim" : "nao";

    // Buscar radios dentro do container correto
    const pressurizacaoRadios = inputsContainer.querySelectorAll(
      `input[type="radio"][name*="pressurizacao"]`,
    );
  ...
  setTimeout(() => {
    // Buscar inputs dentro do container correto
    const textInputs = inputsContainer.querySelectorAll(
    ...
    textInputs.forEach((input) => {
      ...
      if (field === "pressurizacaoSetpoint") {
        ...
        const isPressurizacaoAtiva =
          typeof pressurizacaoValue === "boolean"
            ? pressurizacaoValue
            : pressurizacaoValue === "true" ||
              pressurizacaoValue === true ||
              pressurizacaoValue === 1;
    ...
    // Preencher selects
    const selectInputs = inputsContainer.querySelectorAll(
...
function ensureTableSectionExists(roomElement) {
  ...
  const existingTable = roomElement.querySelector(
    `#subsection-content-${roomId}-input-table`,
  );
  const existingClimaTable = roomElement.querySelector(".clima-table");

  if (existingTable || existingClimaTable) {
    console.log(
      ` Tabela de inputs já existe para sala ${roomElement.dataset.roomName}, ignorando criação`,
    );
    return true;
  }
  ...
  console.log(
    ` Criando tabela de inputs para sala ${roomElement.dataset.roomName}`,
  );
...
function fillThermalGainsData(roomElement, thermalGainsData) {
  ...
  Object.entries(gainSelectors).forEach(([key, selector]) => {
    ...
    if (element && thermalGainsData[key] !== undefined) {
      if (
        key === "total-tr-exato" &&
        typeof thermalGainsData[key] === "number"
      ) {
...
function fillCapacityData(roomElement, capacityData) {
  ...
  const capacidadeUnitariaSelect = document.getElementById(
    `capacidade-unitaria-${roomId}`,
  );
  if (
    capacidadeUnitariaSelect &&
    capacityData.capacidadeUnitaria !== undefined
  ) {
...
/**
 * Garante todas as seções da sala
 */
async function ensureAllRoomSections(roomElement) {
  ...
  try {
    ...
    if (!climatizationExists)
      sectionsToCreate.push({
        type: "climatization",
        func: window.buildClimatizationSection,
      });
    if (!machinesExists)
      sectionsToCreate.push({
        type: "machines",
        func: window.buildMachinesSection,
      });
    if (!acessoriosExists)
      sectionsToCreate.push({
        type: "acessorios",
        func: window.buildAcessoriosSection,
      });
    if (!dutosExists)
      sectionsToCreate.push({ type: "dutos", func: window.buildDutosSection });
    if (!tubosExists)
      sectionsToCreate.push({ type: "tubos", func: window.buildTubosSection });

    console.log(` Criando ${sectionsToCreate.length} seções`);
...
function fixDuplicatedSections(roomElement, sectionTitle) {
  ...
  if (targetSections.length > 1) {
    console.log(
      ` Removendo ${targetSections.length - 1} seções duplicadas de "${sectionTitle}"`,
    );
...
function findMachinesSection(roomElement) {
  return (
    roomElement.querySelector(".machines-section") ||
