...
function extractMachinesData(roomElement) {
  ...
  const machineElements = roomElement.querySelectorAll(
  ...
  console.log(
    ` ${machines.length} máquina(s) extraída(s) da sala ${roomElement.dataset.roomId}`,
  );
  return machines;
}
...
function extractClimatizationMachineData(machineElement) {
  ...
  const machineId =
    machineElement.getAttribute("data-machine-id") || `machine-${Date.now()}`;
  ...
  const machineData = {
    ...
    potencia:
      machineElement.querySelector(".machine-power-select")?.value || "",
    tensao:
      machineElement.querySelector(".machine-voltage-select")?.value || "",
    tensao_comando: "220V",
    quantidade: 1,
    aplicacao_machines: "",
    precoBase: 0,
    ...
    potenciaSelecionada:
      machineElement.querySelector(".machine-power-select")?.value || "",
    tipoSelecionado:
      machineElement.querySelector(".machine-type-select")?.value || "",
  };
  ...
  try {
    ...
    const optionCheckboxes = machineElement.querySelectorAll(
      "#options-container-" + machineId + ' input[type="checkbox"]:checked',
    );

    optionCheckboxes.forEach((checkbox, index) => {
      const optionId =
        checkbox.getAttribute("data-option-id") || (index + 1).toString();
      const optionValue = parseFloat(checkbox.value) || 0;
      const optionName =
        checkbox.getAttribute("data-option-name") || `Opção ${optionId}`;
    ...
    const configCheckboxes = machineElement.querySelectorAll(
      "#config-container-" + machineId + ' input[type="checkbox"]:checked',
    );

    configCheckboxes.forEach((checkbox, index) => {
      const configId =
        checkbox.getAttribute("data-config-id") || (index + 1).toString();
      const configName =
        checkbox.getAttribute("data-config-name") || `Configuração ${configId}`;
    ...
    if (totalPriceElement) {
      ...
      machineData.precoTotal =
        (basePriceUnitario + optionsTotal) * machineData.quantidade;
    }
...
export { extractMachinesData, extractClimatizationMachineData };
