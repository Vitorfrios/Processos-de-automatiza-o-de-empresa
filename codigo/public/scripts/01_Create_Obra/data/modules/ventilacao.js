...
window.scheduleFinishRoomLoading = function (roomId) {
  ...
  pendingFinishTimers[roomId] = setTimeout(() => {
    ...
    console.log(
      ` Modo de carregamento finalizado para sala ${roomId} (após inatividade)`,
    );
...
function finishRoomLoading(roomId, triggerRefresh = true) {
  ...
  if (machinesContainer) {
    ...
    console.log(
      ` Flags de edição manual resetadas para máquinas da sala ${roomId}`,
    );
  }
}
...
function collectRoomInputs(roomId) {
  ...
  if (vazaoArElement) {
    if (
      vazaoArElement.tagName === "DIV" ||
      vazaoArElement.classList.contains("result-value-inline")
    ) {
...
function updateSolutionTable(roomId, inputs) {
  ...
  if (window.isLoadingForRoom && window.isLoadingForRoom(roomId)) {
    console.log(
      ` [updateSolutionTable] Bloqueado - sala ${roomId} em carregamento`,
    );
    return;
  }
  ...
  machines.forEach((machine) => {
    ...
    const tipo =
      tipoSelect.options[tipoSelect.selectedIndex]?.text || "Não definido";

    // Extrai capacidade
    const capacidadeValue = extractCapacidadeValue(capacidadeSelect.value);
    const capacidadeDisplay = capacidadeValue
      ? formatNumber(capacidadeValue)
      : "-";
    ...
    // Inclui o tipo na chave de parâmetros
    ...
    console.log(
      ` [Quantidade] machineId=${machineId}, userEdited=${userEdited}, paramsChanged=${paramsChanged}`,
    );
    console.log(` lastParams: ${lastParams}`);
    console.log(` currentParams: ${currentParams}`);
    console.log(
      ` solucaoNumerica: ${solucaoNumerica}, valorAtual: ${qntInput.value}`,
    );
    ...
    if (paramsChanged) {
      ...
      if (!userEdited) {
        ...
        console.log(
          ` [Ventilação] Parâmetros mudaram. Quantidade da máquina ${machineId} ATUALIZADA para ${solucaoNumerica} (automática)`,
        );
        ...
        console.log(
          ` [Ventilação] Parâmetros mudaram, mas quantidade manual da máquina ${machineId} preservada: ${qntInput.value}`,
        );
      }
    } else {
      ...
      if (!userEdited) {
        ...
        if (currentValue !== solucaoNumerica) {
          qntInput.value = solucaoNumerica;
          console.log(
            ` [Ventilação] Quantidade da máquina ${machineId} ajustada para solução: ${solucaoNumerica}`,
          );
          if (window.calculateMachinePrice) {
            window.calculateMachinePrice(machineId);
          }
        ...
        console.log(
          ` [Ventilação] Quantidade manual da máquina ${machineId} preservada: ${qntInput.value}`,
        );
      }
    }
...
window.handleManualQuantityEdit = function (machineId) {
  ...
  if (roomId && isLoadingForRoom(roomId)) {
    console.log(
      ` [VENTILAÇÃO] Edição manual ignorada - sala ${roomId} em carregamento`,
    );
    return;
  }
  ...
  if (roomId) {
    ...
    if (VALID_APPLICATIONS.includes(aplicacao)) {
      const vazaoNecessaria = calculateVazaoByAplicacao(
        aplicacao,
        roomId,
        inputs,
      );
  ...
  console.log(
    ` [Ventilação] Usuário editou manualmente quantidade da máquina ${machineId} para ${qntInput.value}`,
  );
  ...
  const observer = new MutationObserver((mutations) => {
    ...
    mutations.forEach((mutation) => {
      ...
      if (mutation.type === "attributes") {
        ...
        if (
          target.id?.startsWith("aplicacao-") ||
...
window.refreshVentilationForRoom = function (roomId) {
  ...
  if (window.isLoadingForRoom && window.isLoadingForRoom(roomId)) {
    console.log(
      ` [VENTILAÇÃO] Cálculo bloqueado - sala ${roomId} em carregamento`,
    );
    return;
  }
...
window.handleVentilacaoAplicacaoChange = function (machineId) {
  ...
  if (roomId && isLoadingForRoom(roomId)) {
    console.log(
      ` [VENTILAÇÃO] Aplicação alterada ignorada - sala ${roomId} em carregamento`,
    );
    return;
  }
...
window.handleVentilacaoPowerChange = function (machineId) {
  ...
  if (roomId && isLoadingForRoom(roomId)) {
    console.log(
      ` [VENTILAÇÃO] Capacidade alterada ignorada - sala ${roomId} em carregamento`,
    );
    return;
  }
...
window.handleVentilacaoTipoChange = function (machineId) {
  ...
  if (roomId && isLoadingForRoom(roomId)) {
    console.log(
      ` [VENTILAÇÃO] Tipo alterado ignorado - sala ${roomId} em carregamento`,
    );
    return;
  }
...
function fillVentilacaoInputs(roomElement, ventilacaoData) {
  ...
  console.log(
    ` Preenchendo inputs de ventilação para sala ${roomId}`,
    ventilacaoData,
  );
...
function buildVentilacaoSection(roomId) {
  ...
  if (!roomId.includes("_proj_") || !roomId.includes("_sala_")) {
    console.warn(` ID não parece ser de sala: ${roomId}`);
    return "";
  }
