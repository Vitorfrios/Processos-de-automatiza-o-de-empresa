...
// Módulo de gerenciamento de Dutos
...
function renderDutosTable() {
 ...
 `;
  ...
  dutosData.forEach((duto, index) => {
    ...
    const opcionaisCount =
      duto.opcionais && Array.isArray(duto.opcionais)
        ? duto.opcionais.length
        : 0;
...
export function refreshDutoTableRow(index) {
  ...
  const targetRow = dutosTableBody.querySelector(
    `tr[data-duto-index="${index}"]`,
  );
...
export function updateDutoOpcional(dutoIndex, opcionalIndex, field, value) {
  if (dutosData[dutoIndex] && dutosData[dutoIndex].opcionais) {
    // Garantir que valor seja número quando apropriado
    if (field === "value") {
      dutosData[dutoIndex].opcionais[opcionalIndex][field] =
        parseFloat(value) || 0;
    } else {
    ...
    if (field === "nome") {
      const card = document.querySelector(
        `.opcional-card[data-index="${opcionalIndex}"] .card-title-input`,
      );
...
export function resetDutoChanges() {
  if (
    confirm(
      "Descartar todas as alterações nos dutos? Esta ação não pode ser desfeita.",
    )
  ) {
    ...
    setTimeout(() => {
      if (typeof initDutosModule === "function") {
        initDutosModule().catch((error) => {
          console.error(
            " Falha na inicialização automática do módulo de dutos:",
            error,
          );
        });
      }
