...
class UniversalHTTPRequestHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def handle_post_json_normalize(self):
        ...
        try:
            ...
            # 7. Tubos (NOVA SEÇÃO)
            if isinstance(json_data.get("tubos"), list):
                for tubo in json_data["tubos"]:
                    if isinstance(tubo, dict):
                        normalized_tubo = {
                            "polegadas": tubo.get("polegadas", ""),
                            "mm": tubo.get("mm", 0),
                            "valor": tubo.get("valor", 0),
                            "descricao": tubo.get("descricao", ""),
                        }
                        normalized["tubos"].append(normalized_tubo)
                    elif isinstance(tubo, str):
                        # Se tubo for string, criar estrutura básica
                        normalized["tubos"].append({
                            "polegadas": tubo,
                            "mm": 0,
                            "valor": 0,
                            "descricao": "",
            ...
            self.send_json_response(
                {
                    ...
                    "summary": {
                        ...
                        "dutos_count": len(normalized["dutos"]),
                        "tubos_count": len(normalized["tubos"]),  # ADICIONADO
                        "total_items": (
                            ...
                            len(normalized["dutos"]) +
                            len(normalized["tubos"])  # ADICIONADO
                        )
                    },
