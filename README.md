...
1. Inicie o backend local com `python codigo/servidor.py` (usa a porta 8000 por padr�o e escolhe a pr�xima dispon�vel se j� estiver ocupada; abre o navegador automaticamente para a p�gina `01_Create_Obra`).
2. Confirme que `codigo/json/dados.json`, `codigo/json/backup.json` e `codigo/json/sessions.json` existem; eles s�o criados automaticamente se n�o estiverem presentes.
3. Na SPA, preencha o cabe�alho de empresa: o autocomplete l� `dados.json/empresas` e, se a sigla for nova, o backend cria a entrada automaticamente via `empresa_handler`.
4. Crie/renomeie obras, projetos e salas; os c�lculos s�o disparados conforme voc� digita e os resultados alimentam a tabela de capacidade e os cards de m�quinas.
...
6. Use `02_Obras_manager` para revisar obras j� salvas (`/api/backup-completo`) e comparar estat�sticas; o m�dulo reutiliza c�lculos da p�gina principal.
7. Para finalizar, acione o bot�o de desligamento ou chame `/api/sessions/shutdown` + `/api/shutdown` para limpar sess�o, fechar threads e encerrar o servidor.

## Convencoes de IDs e formatos basicos

- Prefixos: `obra_*`, `project_*` e `room_*` s�o gerados por `data/utils/id-generator.js` e validados tanto no front quanto no backend (`SessionsManager` e rotas `/obras`).
- IDs compostos seguem o padr�o `obra_<sigla>_<sequencia>`, `obra_<sigla>_<seq>_proj_<hash>_<n>` e `..._sala_<hash>_<n>` para manter unicidade em m�ltiplas cria��es r�pidas.
...
- `sessions.json` mant�m apenas IDs (`{"sessions":{"session_active":{"obras":[]}}}`); o conte�do real permanece em `backup.json`.
- `dados.json` armazena `constants`, `machines`, `empresas` (lista de objetos `{SIGLA: Nome}`) e custos auxiliares usados nos c�lculos.
...
- Campos opcionais podem ser omitidos, mas o front espera sempre `inputs`, `maquinas`, `capacidade`, `ganhosTermicos` e `configuracao` para evitar erros de renderiza��o.
- `somavaloresmaquinatotal` armazena o somat�rio final dos pre�os das m�quinas quando calculado.

### dados.json (constants, machines, empresas)

- `constants`: blocos num�ricos usados nos c�lculos de ganhos e capacidade (`VARIAVEL_PD`, `VARIAVEL_PS`, `deltaT_*`, `FATOR_SEGURANCA_CAPACIDADE` etc).
- `machines`: cat�logo por tipo com impostos, op��es extras, configura��es de instala��o e faixas de pot�ncia/pre�o; o front usa para montar dropdowns e calcular `precoTotal`.
...
- Outras chaves incluem custos unit�rios de materiais e impostos auxiliares reutilizados em c�lculos e tooltips.

### sessions.json (controle de sess�o)
...
- Nenhum dado sens�vel � salvo aqui; serve apenas para filtrar quais obras do `backup.json` podem ser carregadas pela sess�o atual.

## Endpoints HTTP (backend Python)

### Obras
- `GET /obras`: retorna obras da sess�o ativa (filtradas de `backup.json`).
- `GET /obras/{id}`: busca obra espec�fica.
- `POST /obras`: cria obra; body � o objeto completo (ver `backup.json`); gera ID se faltando.
- `PUT /obras/{id}`: atualiza obra existente; sobrescreve o registro no `backup.json`.
- `DELETE /obras/{id}`: remove obra do backup e, se chamada via UI, dispara reload para limpar a sess�o.

### Empresas
- `GET /api/dados/empresas`: lista `{SIGLA: Nome}`.
- `GET /api/dados/empresas/numero/{sigla}`: devolve pr�ximo n�mero sequencial de cliente para a sigla.
...
- `DELETE /api/sessions/remove-obra/{id}`: remove ID da sess�o; usado ap�s confirma��es/undos.
- `POST /api/sessions/shutdown`: limpa sess�o ativa (lista de obras).
- `POST /api/sessions/ensure-single`: for�a apenas uma sess�o ativa (cleanup defensivo).

### Dados auxiliares e sistema
- `GET /constants`: retorna `dados.json.constants`.
- `GET /machines`: retorna `dados.json.machines`.
- `GET /dados` / `POST /dados`: l�/salva `dados.json` completo.
- `GET /backup` / `POST /backup`: l�/salva `backup.json` completo.
- `GET /api/backup-completo`: retorna todas as obras sem filtrar por sess�o.
- `POST /api/reload-page`: endpoint de compatibilidade usado pelos modais de undo para sinalizar recarregamento.
- `GET /api/server/uptime`: informa tempo em execu��o do backend.
- `POST /api/shutdown`: encerra o servidor Python de forma graceful.

## Fluxo de sessao e persistencia

- Bootstrap (`system-init.js`) baixa constantes (`/constants`), carrega m�dulos e, em seguida, `session-manager-main.js` chama `/api/session-obras` para recuperar IDs v�lidos.
...
- Exclus�es disparam `/api/sessions/remove-obra/{id}` e `/api/reload-page` (para limpar DOM e estado), respeitando o sistema de undo da UI.
- O bot�o de desligamento usa `shutdown-adapter` para chamar `/api/sessions/shutdown` (limpa `sessions.json`) e depois `/api/shutdown` (encerra threads/HTTP server).

## Logs, diagnostico e monitoramento

- Front-end: `core/logger` intercepta `console.*`; use `window.toggleLogger()` e `window.getLoggerStatus()` para ligar/desligar o logger inteligente. `finalSystemDebug` imprime estado global ap�s o carregamento.
- Backend: mensagens em `servidor.py` e `server_core.py` indicam porta, URL e tempo de uptime; handlers mostram �cones para a��es de obra/empresa e erros.
- Arquivos auxiliares: `arquivostxt/status.txt` pode armazenar status manuais; `arquivostxt/relatorio_js_detalhado.txt` registra mapeamento de fun��es JS gerado pelos utilit�rios Python.
- Diagn�stico r�pido: consulte `codigo/json/backup.json` para verificar se a obra foi persistida e `codigo/json/sessions.json` para confirmar se a sess�o cont�m o ID salvo.
- Para limpar caches Python, `cache_cleaner` roda em thread; se precisar for�ar, remova `__pycache__` manualmente ou reinicie o servidor.

## Checklist de testes manuais

- Subir o servidor e acessar a p�gina 01; confirmar carregamento das constantes (sem status vermelho).
- Criar nova obra com empresa nova: preencher sigla/nome, salvar e confirmar que a sigla aparece em `dados.json` e no cabe�alho da obra.
- Adicionar m�ltiplos projetos/salas, preencher campos de climatizacao, conferir rec�lculo autom�tico de vaz�o/ganhos/capacidade e valores exibidos nos cards de m�quinas.
- Salvar obra, recarregar p�gina e validar restaura��o completa (dados de empresa, projetos, salas, m�quinas e banners de status).
- Abrir `02_Obras_manager`, verificar listagem e filtros, e conferir cores/�cones (`powerMatch`) para vaz�o.
- Executar fluxo de exclus�o com modal, testar undo e confirmar que `sessions.json` e `backup.json` refletem o resultado final.

## Troubleshooting rapido

- Porta ocupada: o servidor tenta matar processos na porta padr�o; se falhar, ele escolhe a pr�xima dispon�vel. Verifique se outra inst�ncia do app est� rodando.
- SPA n�o carrega m�dulos: confira se `/constants` est� respondendo; se n�o, `dados.json` pode estar corrompido (rode `python -m json.tool codigo/json/dados.json` para validar).
- Autocomplete de empresa vazio: certifique-se de que `dados.json` possui a chave `empresas` em formato de lista de objetos e que o backend est� respondendo a `/api/dados/empresas`.
- Sess�o presa (obras n�o removem): chame manualmente `/api/sessions/shutdown` para limpar `sessions.json` e recarregue a p�gina.
- Persist�ncia falhando: verifique permiss�es de escrita na pasta `codigo/json`; o backend usa `FileUtils.save_json_file` e avisar� no console se n�o conseguir gravar.

## Implantacao e servico

- Para produ��o/render, use `app.py` (Flask) que serve os assets est�ticos e mant�m health check; a URL padr�o continua `http://<host>:<port>/codigo/public/pages/01_Create_Obra/index.html`.
- `runtime.txt` e `rander.yml` definem a vers�o de Python e configura��es de deploy; `CNAME` aponta dom�nio personalizado caso configurado.

## Scripts auxiliares

- `utilitarios py/fun��es.py`: gera templates base de builders (`data/builders/data-builders-folder`) para acelerar refatora��es; execute em um ambiente virtual para n�o poluir depend�ncias globais.
- Scripts adicionais na pasta `utilitarios py/` podem auditar pastas, juntar linhas ou criar relat�rios; revise cada cabe�alho para entender par�metros antes de rodar.
